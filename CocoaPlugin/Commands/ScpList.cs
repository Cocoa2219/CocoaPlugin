...
public class ScpList : ICommand, IHelpableCommand
{
    public bool Execute(ArraySegment<string> arguments, ICommandSender sender, [UnscopedRef] out string response)
    {
        ...
        {
            sb.AppendLine(scp.HasCustomName ? $"<color=white><color={scp.GetRoleColor()}>{scp.GetRoleName()}</color> | {scp.DisplayNickname} ({scp.Nickname}) | {scp.GetUserId()}</color>" : $"<color=white><color={scp.GetRoleColor()}>{scp.GetRoleName()}</color> | {scp.Nickname} | {scp.UserId}</color>");
        }
