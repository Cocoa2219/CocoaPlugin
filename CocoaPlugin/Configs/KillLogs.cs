...
public class KillLogs
{
    public Dictionary<KillType, API.Broadcast> KillLog { get; set; } = new()
    {
        ...
        { KillType.Hypothermia, new API.Broadcast("<cspace=0.05em><size=24><color=%targetRoleColor%>%targetRoleName%<b> %targetNickname%</b></color>%targetNicknameParticle% <color=#d44b42>☃️ 얼어 죽었습니다.</color></size></cspace>", 10)},
        ...
        { KillType.ParticleDisruptor, new API.Broadcast("<cspace=0.05em><size=24><color=%targetRoleColor%>%targetRoleName%<b> %targetNickname%</b></color>%targetNicknameParticle% <color=%attackerRoleColor%>%attackerRoleName%<b> %attackerNickname%</b></color>에 의해 <color=#d44b42>⚡ 심한 전기 화상</color>을 입었습니다.</size></cspace>", 10)},
